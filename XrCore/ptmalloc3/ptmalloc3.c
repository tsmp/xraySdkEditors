...
#define MSPACE_OFFSET (((offsetof(struct malloc_arena, buf_) + CHUNK_ALIGN_MASK) & ~CHUNK_ALIGN_MASK))
...
#define arena_for_chunk(ptr)                                                                                               \
  (chunk_non_main_arena(ptr) ? *(struct malloc_arena **)((char *)(ptr) + chunksize(ptr) - (FOOTER_OVERHEAD - SIZE_T_SIZE)) \
                             : &main_arena)

/* special because of extra overhead */
#define arena_for_mmap_chunk(ptr)                                                                                        \
  (chunk_non_main_arena(ptr) ? *(struct malloc_arena **)((char *)(ptr) + chunksize(ptr) - sizeof(struct malloc_arena *)) \
                             : &main_arena)

#define set_non_main_arena(mem, ar_ptr)                                             \
  do                                                                                \
  {                                                                                 \
...
void weak_variable (*__free_hook)(void *__ptr, const void *) = free_hook_ini;
...
int public_pMEMALIGn(void **memptr, size_t alignment, size_t size)
{
  ...
  if (alignment % sizeof(void *) != 0 || !my_powerof2(alignment / sizeof(void *)) != 0 || alignment == 0)
    return EINVAL;
